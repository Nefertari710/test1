    ...
    public partial class Cache : Form
    {
            ...
            while (true)
            {
                ...
                if (command == 3) {
                    ...
                    using (NetworkStream stream = tcpClientCache3.GetStream())
                    {
                        ...
                        // 保安来了
                        // 告诉Client后面要读取多少个块
                        streamClient.Write(pieceLength, 0, pieceLength.Length);
                        ...
                        for (int i = 0; i < PieceLengthBytes; i++)
                        {
                            ...
                            if (PieceType == 0)
                            {
                                ...
                                // 处理子文件夹中的文件
                                string[] fileEntries = Directory.GetFiles(PieceDataFilePath);
                                foreach (string filePath in fileEntries)
                                {

                                    string HashPieceContentString = Encoding.UTF8.GetString(HashPieceContentBytes);

                                    Invoke((Action)(() => textBoxData.AppendText(HashPieceContentString)));
                                    if (Path.GetFileName(filePath) == HashPieceContentString)
                                    {
                                        //相同hash应该将文件块传回来

                                        string ContentFilePath = File.ReadAllText(filePath);
                                        byte[] ContentFilePathByte = Encoding.UTF8.GetBytes(ContentFilePath);
                                        byte[] ContentFilePathByteLength = BitConverter.GetBytes(ContentFilePathByte.Length);
                                        byte[] ContentFilePathCombine = new byte[4 + ContentFilePathByte.Length];
                                        Array.Copy(ContentFilePathByteLength, 0, ContentFilePathCombine, 0, ContentFilePathByteLength.Length);
                                        Array.Copy(ContentFilePathByte,0,ContentFilePathCombine, ContentFilePathByteLength.Length, ContentFilePathByte.Length);
                                        
                                        streamClient.Write(ContentFilePathCombine,0,ContentFilePathByte.Length);
                                        streamClient.Flush();
                            ...
                            if (PieceType == 1)
                            {
                                ...
                                Invoke((Action)(() => textBoxData.AppendText(TempHash)));
                                ...
                                byte[] HashPieceContentBytesCombine = new byte[4 + HashPieceLengthInt];
                                Array.Copy(HashPieceLength , 0, HashPieceContentBytesCombine, 0, HashPieceLength.Length);
                                Array.Copy(HashPieceContentBytes, 0, HashPieceContentBytesCombine, HashPieceLength.Length, HashPieceContentBytes.Length);

                                streamClient.Write(HashPieceContentBytesCombine, 0, HashPieceContentBytesCombine.Length);
                                streamClient.Flush();
                            }

                        }
                        double percentage = (double)notttt / (double)PieceLengthBytes * 100;
                        double roundedNumberpercentage = Math.Round(percentage, 2);


                        // 没什么用就是单纯的处理维护一下
                        logger1.Log(roundedNumberpercentage.ToString() + "% of file " + fileName + " was constructed with the cached data.", fileName);
                        Log1(roundedNumberpercentage.ToString() + "% of file " + fileName + " was constructed with the cached data.", fileName);
                            ...
                            if (PieceType == 0)
                            {
                                ...
                                foreach (string filePath in fileEntries)
                                {
                                    ...
                                    if (Path.GetFileName(filePath) == HashPieceContentString)
                                    {
                                        ...
                                        streamClient.Write(ContentFilePathCombine, 0, ContentFilePathCombine.Length );
                                        streamClient.Flush();
        ...
        private void buttonDeleteAllFile_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                labelDeleteFIleStatus.Text = "Delete Complete!";
            }
            catch (Exception ex)
            {
