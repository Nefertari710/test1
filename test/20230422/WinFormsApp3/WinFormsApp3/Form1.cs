    ...
    public partial class Server : Form
    {
        ...
        private void buttonInsertFile_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                // 在此处执行处理所选文件的操作
                string piecefilePath0 = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "../../../ServerAllData", Path.GetFileName(selectedFilePath)));

                File.Copy(selectedFilePath, piecefilePath0, true);

                piece_Data();

                listBoxAllData.Items.Add(Path.GetFileName(selectedFilePath));
            }
        }
    }
}
